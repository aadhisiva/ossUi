...
export default function WardComponent() {
  ...
  const [{ userRole, Mobile, userCodes }] = IsAuthenticated();
  ...
  const handleCLickAdd = async () => {
    ...
    let find: any = originalData.find((obj) => obj.DistrictName == district && obj.TalukName === taluk && obj.GramPanchayatName == panchayat);
    setFormData(find);
    setEditForm(true);
    setModalTitle('Add');
  };
  ...
  const handleTalukSelect = (value: string) => {
    if(taluk !== value){
      setTaluk(value);
      setPanchayat("");
      let gpSelect: any = Array.from(new Set(((originalData).filter((obj) => obj.TalukName === value).map(obj => obj.GramPanchayatName))));
