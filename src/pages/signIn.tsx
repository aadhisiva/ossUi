  ...
  const handleSubmitOtp = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === true) {
      ...
      if (usersUniqueList?.length > 1) {
        ...
        } else if (fetchRole == ROLES.DISTRICT_OFFICER) {
          let codes = Array.from(
            ...
            <Row className="mb-4 flex flex-col">
              ...
              />
                <div className="flex justify-end">
                  <a onClick={handleResendOtp} className={`${timer < 1 ? "text-red-400 cursor-pointer" : "pointer-events-none text-gray-600" }`}>Resend Otp {timer > 0 && `(${timer})`}</a>
                  </div>
              <CustomCaptch
